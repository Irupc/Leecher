    ...
    if to_upload_file:
        if CUSTOM_FILE_NAME:
            ...
            if os.path.isfile(to_upload_file):
                ...
                fName_mod = fName_mod.replace("PSA", randletter).replace("YIFY", randletter).replace("GalaxyRG", randletter).replace("[YTS.LT]", randletter).replace("[YTS.MX]", randletter).replace("YTS", randletter).replace("[FilmsZilla.com]", randletter).replace("x265lk", randletter).replace("'", "").replace(",", ".").replace("\"", "")
